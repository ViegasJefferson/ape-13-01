...
import { UpdateExpenseDialog } from "@/features/gastos/components/update-expense-dialog";
import {
  Card,
  CardContent,
  CardDescription,
...
function getStatusBadge(status: ExpenseStatus) {
  if (status === "paid") {
    return <Badge className="bg-emerald-100 text-emerald-950">Pago</Badge>;
  }

  if (status === "overdue") {
    return <Badge className="bg-red-100 text-red-900">Vencido</Badge>;
  }

  if (status === "partial") {
    return <Badge className="bg-amber-100 text-amber-900">Parcial</Badge>;
  }
...
export function ExpenseDashboard({
  ...
  const currentMonthPaid = data.expenses.reduce((total, expense) => {
    if (!expense.paidAt || !expense.paidAt.startsWith(currentYearMonth)) {
      return total;
    }
  ...
  const constructionFeePaid = data.expenses.reduce((total, expense) => {
  ...
  const pendingPlanned = data.expenses.reduce((total, expense) => {
    if (
      expense.status === "paid" ||
    ...
    return total + Math.max(expense.plannedAmount - expense.paidAmount, 0);
  }, 0);
          ...
          <CardContent className="flex flex-col justify-between gap-4 p-5 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-800">
                Vencimento em {formatDatabaseDate(nextExpense.dueDate)}
              </p>
            </div>
            ...
            <div className="text-left sm:text-right">
              ...
              <p className="mt-1 text-xl font-semibold text-emerald-950">
                {nextExpense.plannedAmount === null
                  ? "A informar"
                  : formatCurrency(nextExpense.plannedAmount)}
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          {data.expenses.length === 0 ? (
            <div className="rounded-xl border border-dashed p-8 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Os próximos lançamentos aparecerão nesta área.
              </p>
            </div>
          ) : (
                  ...
                  {data.expenses.map((expense) => (
                    <TableRow key={expense.id}>
                      <TableCell className="whitespace-nowrap font-medium">
                        {formatDatabaseDate(expense.dueDate)}
                      </TableCell>
